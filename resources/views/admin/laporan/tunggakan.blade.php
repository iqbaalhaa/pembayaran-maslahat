...
<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <div>
            <h2 style="font-size: 1.25rem; font-weight: 700; margin: 0;">Laporan Tunggakan</h2>
            <p style="color: var(--muted); margin: 4px 0 0; font-size: 0.9rem;">Daftar tagihan yang belum lunas atau menunggu konfirmasi.</p>
        </div>
    </div>

    <div style="margin-bottom: 20px; background: #f9fafb; padding: 15px; border-radius: 8px; border: 1px solid var(--border);">
        <form action="{{ route('laporan.tunggakan') }}" method="GET" style="display: flex; gap: 10px; flex-wrap: wrap; align-items: flex-end;">
            ...
            <div style="display: flex; gap: 8px; align-items: center;">
                ...
                <a href="{{ route('laporan.tunggakan.export', request()->query()) }}" class="btn btn-primary" style="height: 42px; display: inline-flex; align-items: center; gap: 6px;">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="width: 20px; height: 20px;">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                    </svg>
                    Excel
                </a>
                <a href="{{ route('laporan.tunggakan.export-pdf', request()->query()) }}" class="btn btn-danger" style="height: 42px; display: inline-flex; align-items: center; background: #dc2626; border-color: #dc2626; color: white; gap: 6px;">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="width: 20px; height: 20px;">
    ...
    <div style="margin-bottom: 16px; padding: 12px 16px; border-radius: 8px; background: #fef3c7; border: 1px solid #fde68a; display: flex; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
        <div>
            <div style="font-size: 0.85rem; color: var(--muted);">Periode</div>
            <div style="font-weight: 600;">{{ $selectedMonth ?? 'Semua' }} {{ $selectedYear ?? '' }}</div>
        </div>
        <div>
            <div style="font-size: 0.85rem; color: var(--muted);">Total Tunggakan</div>
            <div style="font-weight: 600;">{{ $totalTransaksi }} item</div>
        </div>
        <div>
            <div style="font-size: 0.85rem; color: var(--muted);">Total Nominal</div>
            <div style="font-weight: 700; color: #b45309;">Rp {{ number_format($totalJumlah, 0, ',', '.') }}</div>
        </div>
    </div>
        ...
        <table class="table" style="width: 100%; border-collapse: collapse; text-align: left;">
            <thead>
                <tr>
                    ...
                    <th style="padding: 10px;">Jenis Tagihan</th>
                    <th style="padding: 10px;">Bulan/Tahun</th>
            ...
            <tbody>
                ...
                @empty
                <tr>
                    <td colspan="9" style="padding: 12px; text-align: center; color: var(--muted);">Tidak ada data tunggakan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
